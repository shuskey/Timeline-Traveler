...
namespace Assets.Scripts.DataProviders
{
    public class ListOfTopEventsFromDataBase
    {
        ...
        public void GetListOfTopEventsFromDataBase(int? specificYear = null)
        {
            ...
            {
                Debug.LogWarning($"TopEvents database file not found at: {_dataBaseFileName}. No historical events will be displayed.");
                return;
            }
            ...
            {
                Debug.LogError($"Error accessing TopEvents database: {ex.Message}. No historical events will be displayed.");
                topEventsList.Clear(); // Ensure the list is empty on error
            }
        }
    }        
}
